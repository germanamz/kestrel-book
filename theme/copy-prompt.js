(function copyPromptButtons() {
    const COPY_ICON_SVG =
        '<svg viewBox="0 0 24 25" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    ...
    const blockquotes = document.querySelectorAll('.callout blockquote');

    blockquotes.forEach(function (blockquote) {
        blockquote.classList.add('has-copy-prompt');

        const button = document.createElement('button');
        button.className = 'copy-prompt-button';
        button.type = 'button';
        button.setAttribute('aria-label', 'Copy prompt to clipboard');
        button.setAttribute('title', 'Copy prompt to clipboard');
        button.innerHTML = COPY_ICON_SVG;

        button.addEventListener('click', function () {
            const text = blockquote.innerText.trim();
            navigator.clipboard.writeText(text).then(
                function () { flashTitle(button, 'Copied!'); },
                function () { flashTitle(button, 'Clipboard error!'); }
            );
        });
    ...
    function flashTitle(button, msg) {
